...
def init_weights(shape, name, sample = "xavier", couple_axis = None):
    if couple_axis is None:
        if sample == "norm":
            values = norm_weight(shape, 0.01)
        elif sample == "xavier":
    ...
    elif couple_axis in [0, 1]:
        if sample == "norm":
            values = np.concatenate([norm_weight(shape, 0.01), norm_weight(shape, 0.01)], couple_axis)
        elif sample == "xavier":
